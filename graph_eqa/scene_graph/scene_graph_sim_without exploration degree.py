...
class SceneGraphSim:
    ...
    def get_current_semantic_state_str(self):
        ...
        if self.include_regions:
            ...
            if agent_place_ids:  #  添加检查：确保列表不为空
                # 获取区域所在的房间节点
        ...
        if len(room_id) > 0:
            room_name = self.filtered_netx_graph.nodes[room_id[0]].get('name', 'room')
            if room_name != 'room':
    ...
    def get_room_for_object(self, obj_id):
        """
        获取对象节点所属的房间节点ID
        
        Args:
            obj_id: 对象节点ID（如 'object_12'）
        Returns:
            房间节点ID（如 'room_2'）或 None
        """
        if obj_id not in self.filtered_netx_graph:
            return None
        
        # 方法1：查找对象节点的前驱节点
        predecessors = list(self.filtered_netx_graph.predecessors(obj_id))
        
        for pred_id in predecessors:
            # 如果对象直接连接到区域节点
            if 'region' in pred_id:
                # 再查找区域节点的前驱（应该是房间）
                region_predecessors = list(self.filtered_netx_graph.predecessors(pred_id))
                for room_id in region_predecessors:
                    if 'room' in room_id:
                        return room_id
            
            # 如果对象直接连接到房间节点（在区域被移除的情况下）
            elif 'room' in pred_id:
                return pred_id
